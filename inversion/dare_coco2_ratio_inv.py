...
#mixing_thresh= {"CO2":0.1,
#                "CO": 0.5} # Permitted std. dev of model concs in lowest 5 model layers. 

fix_gee = False
scale_bio_uncert = False
...
prior_scaling = 1./monthly_ratios[month-1]
#prior_scaling = 1.2
prior_bio_scaling = 0.5

bio_ratios = np.asarray([5., 4., 3., 2., 1., 1., 1.,
       1., 2., 2., 4., 5.])
bio_sigma_scale = bio_ratios[month-1]
...
if well_mixed == True:
    ...
    #wh_mixed2_co2 = np.where(np.abs(y_slope_co2) < mixing_thresh2["CO2"])[0]
    ...
    #wh_mixed2_co = np.where(np.abs(y_slope_co) < mixing_thresh2["CO"])[0]
    ...
    #wh_mixed2 = np.hstack([wh_mixed2_co2, wh_mixed2_co+nobs_unfilt_co2])
...
#print("Unfiltered: ", len(y_obs_temp))
#print ("Filtered: ", nobs, nobs_co2, nobs_co)
#%%  
...
if scale_bio_uncert == True:
    wh_bio = np.where((x_group == "reco") | (x_group == "gee") | (x_group == "bio"))[0]
    ad_pdf_param2[wh_bio] = ad_param2_0*bio_sigma_scale


if fix_gee == True:
    ad_pdf_param2[wh_co2gee]  = 0.01
    stepsize_ad[wh_co2gee] = 0.
