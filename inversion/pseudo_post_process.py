...
#version = "pseudo_paper_coco2_v2"
#version = "pseudo_paper_coco2_fixed_v2"
#version = "pseudo_paper_co2_only_v2"

#version = "pseudo_coco2_corr1_sigma2"
#version = "pseudo_coco2_corr1_sigma5"
#version = "pseudo_coco2_corr1_sigma10"

#version = "pseudo_coco2_corr08_sigma2"
#version = "pseudo_coco2_corr08_sigma5"
#version = "pseudo_coco2_corr08_sigma10"

#version = "pseudo_coco2_corr04_sigma2"
#version = "pseudo_coco2_corr04_sigma5"
#version = "pseudo_coco2_corr04_sigma10"

version = "pseudo_co2_only_v4"
#version = "pseudo_coco2_v2_corr04_sigma10"
...
for run_date in run_dates:
    ...
    for spc in species:
        ...
        for group in spc_groups[spc]:
            ...
            emis_true[group] = ds_hemco[emis_keys[spc][0]]*0.
            ...
            for xi, reg in enumerate(emis_keys[spc]):
                ...
                emis_true[group] = emis_true[group] + emis_bf*ds_inv_out["x_true_" + spc.lower() +group][xi]
        ...
        # need to combine each grouped run_date map into a list I guess
            ...
            emis_true_list[spc.lower() + group].append(emis_true[group])   
...
for group in run_groups:
    ...
    da_emis_true[group] = xarray.concat(emis_true_list[group], dim="time")
...
mask = regionmask.defined_regions.natural_earth_v5_0_0.countries_110.mask(lon_state, lat_state)
...
for group in run_groups:
    ...
    emis_cnt_true[group] = xarray.DataArray(np.zeros((ncountries,ntime)), 
               coords=[countries, pd_rdates], dims=["country", "time"])
...
for ci,country in enumerate(countries):
    ...
    for group in run_groups:
        ...
        emis_cnt_dum3 = da_emis_true[group].where(mask==c_idx)
        ...
        emis_cnt_true[group][ci,:] = (emis_cnt_dum3*da_area).sum(dim=["lat","lon"])*t_convert/1.e9 # Tg/yr
...
for group in run_groups:
    ...
    ds_out["country_true_"  + group] =  emis_cnt_true[group]
    ...
    ds_out["emis_true_" + group] = da_emis_true[group]
    ...
    ds_out["country_true_"  + group].attrs["units"] = "Tg/yr"
    
    ds_out["emis_mean_"  + group].attrs["units"] = "kg/m2/s"
    ds_out["emis_ap_"  + group].attrs["units"] = "kg/m2/s"
    ds_out["emis_true_"  + group].attrs["units"] = "kg/m2/s"
